...
if ($selectedUserId > 0) {
    ...
    $stmt = $conn->prepare("SELECT date, calories_in, calories_out FROM calories_tracker WHERE user_id = ? AND WEEK(date, 1) = WEEK(CURDATE(), 1)");
    $stmt->bind_param("i", $selectedUserId);
    $stmt->execute();
...
<head>
  <meta charset="UTF-8">
  <title>Thống kê calo theo người dùng</title>
...
<body>
  <h2>Thống kê calo theo người dùng</h2>
