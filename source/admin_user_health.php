...
$weekData = array_fill_keys($days, ['sys' => null, 'dia' => null, 'hr' => null]);
...
if ($selectedUserId > 0) {
    ...
    $stmt = $conn->prepare("SELECT date, ROUND(AVG(systolic)) AS sys, ROUND(AVG(diastolic)) AS dia, ROUND(AVG(heart_rate)) AS hr FROM health_metrics WHERE user_id = ? AND WEEK(date, 1) = WEEK(CURDATE(), 1) GROUP BY date");
    $stmt->bind_param("i", $selectedUserId);
    $stmt->execute();
...
$hrArray  = [];
foreach ($days as $d) {
    $sysArray[] = $weekData[$d]['sys'];
    $diaArray[] = $weekData[$d]['dia'];
    $hrArray[]  = $weekData[$d]['hr'];
}
...
<head>
  <meta charset="UTF-8">
  <title>Thống kê huyết áp & nhịp tim người dùng</title>
  <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
  <style>
    body { font-family: 'Segoe UI', sans-serif; padding: 40px; background: #f0f9ff; }
    h2 { color: #1a92be; text-align: center; margin-bottom: 30px; }
    form { text-align: center; margin-bottom: 30px; }
    select, button { padding: 8px 14px; font-size: 16px; border-radius: 6px; border: 1px solid #ccc; }
    button { background-color: #1a92be; color: white; font-weight: bold; cursor: pointer; margin-left: 10px; }
    table { width: 100%; max-width: 1000px; margin: auto; border-collapse: collapse; background: white; box-shadow: 0 0 10px rgba(0,0,0,0.1); margin-bottom: 40px; }
    th, td { padding: 12px; text-align: center; border: 1px solid #ccc; }
    th { background-color: #1a92be; color: white; }
    .chart-container { width: 90%; max-width: 1000px; margin: auto; }
  </style>
</head>
<body>
  <h2>Thống kê huyết áp & nhịp tim theo người dùng</h2>
    ...
    <table>
      ...
      <tr>
        ...
        <?php foreach ($days as $d): ?>
          <td>
            <?= $weekData[$d]['sys'] && $weekData[$d]['dia'] ? $weekData[$d]['sys'] . '/' . $weekData[$d]['dia'] : '-' ?><br>
            <?= $weekData[$d]['hr'] ? $weekData[$d]['hr'] . ' bpm' : '' ?>
          </td>
        <?php endforeach; ?>
      </tr>
    </table>
